...
Route::match(['get', 'post'], '/admin/classroom-allocation', function (Illuminate\Http\Request $request) {
    ...
    if ($request->isMethod('post')) {
        if ($request->input('form_type') === 'auto-allocate' || $request->input('form_type') === 're-generate') {
            ...
            // Only consider users with an enrollment_number (actual students)
            ...
            if ($classrooms->isEmpty()) {
                // Determine if any lab or classroom
            ...
            foreach ($classes as $cls) {
                ...
                $capacityToUse = $studentStrength > 0 ? $studentStrength : 60; // Fallback capacity just in case
